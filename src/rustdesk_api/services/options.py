...
def _accounts(s: Settings) -> list[Option]:
    ...
    return [
        ...
        Option(
            ...
            "A sign-in that does not match a local password is tried against a directory instead.",
            s.ldap_enabled,
            "directory at {1}" if s.ldap_enabled else None,
            (s.ldap_server_url,) if s.ldap_enabled else (),
        ),
        Option(
            ACCOUNTS,
