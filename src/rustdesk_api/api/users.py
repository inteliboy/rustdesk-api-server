...
def list_users(db: Session = Depends(get_db), _admin: User = Depends(get_current_admin)) -> list[User]:
...
def create_user(
    payload: CreateUserRequest, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)
) -> User:
